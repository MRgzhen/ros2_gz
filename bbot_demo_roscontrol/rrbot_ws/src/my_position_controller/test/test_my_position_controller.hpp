...
#ifndef TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_MY_POSITION_CONTROLLER_HPP_
#define TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_MY_POSITION_CONTROLLER_HPP_
...
using ControllerStateMsg = my_position_controller::MyPositionController::ControllerStateMsg;
using ControllerReferenceMsg = my_position_controller::MyPositionController::ControllerReferenceMsg;
using ControllerModeSrvType = my_position_controller::MyPositionController::ControllerModeSrvType;
...
class TestableMyPositionController : public my_position_controller::MyPositionController
{
  FRIEND_TEST(MyPositionControllerTest, all_parameters_set_configure_success);
  FRIEND_TEST(MyPositionControllerTest, activate_success);
  FRIEND_TEST(MyPositionControllerTest, reactivate_success);
  FRIEND_TEST(MyPositionControllerTest, test_setting_slow_mode_service);
  FRIEND_TEST(MyPositionControllerTest, test_update_logic_fast);
  FRIEND_TEST(MyPositionControllerTest, test_update_logic_slow);

public:
  controller_interface::CallbackReturn on_configure(
    const rclcpp_lifecycle::State & previous_state) override
  {
    auto ret = my_position_controller::MyPositionController::on_configure(previous_state);
...
class MyPositionControllerFixture : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerReferenceMsg>(
      "/test_my_position_controller/commands", rclcpp::SystemDefaultsQoS());
    ...
    slow_control_service_client_ = service_caller_node_->create_client<ControllerModeSrvType>(
      "/test_my_position_controller/set_slow_control_mode");
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_my_position_controller")
  {
    ASSERT_EQ(
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_my_position_controller/state", 10, subs_callback);
...
protected:
  ...
  std::unique_ptr<TestableMyPositionController> controller_;
...
#endif  // TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_MY_POSITION_CONTROLLER_HPP_
