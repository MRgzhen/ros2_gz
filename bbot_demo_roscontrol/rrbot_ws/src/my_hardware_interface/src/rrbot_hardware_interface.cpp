  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  joint_states_.resize(info_.joints.size(), std::numeric_limits<double>::quiet_NaN()); //初始化状态节点
  ...
  for (size_t i = 0; i < info_.joints.size(); ++i)
  {
    ...
    state_interfaces.emplace_back(
      hardware_interface::StateInterface(
        ...
        // info_.joints[i].name, hardware_interface::HW_IF_POSITION, &hw_states_[i])  //old_code
